#ifndef GALLERY_FMWK_SUPERA_UBOONE_CLUSTER_CXX
#define GALLERY_FMWK_SUPERA_UBOONE_CLUSTER_CXX
...
void ubooneCluster::initialize() {

}

void ubooneCluster::build_particle_map(gallery::Event* ev, larcv3::IOManager & io) {
  ...
  auto & event_part = io.get_data<larcv3::EventParticle>("ubooneseg");
  ...
  for (auto& shower : *mcshowers) {
    ...
    for (auto& daughter_id : shower.DaughterTrackID()) {
      //std::cout<<"Shower Daughter ID: " << id << std::endl;
...
void ubooneCluster::slice(gallery::Event* ev, larcv3::IOManager & io) {
  ...
  auto & event_cluster3d = io.get_data<larcv3::EventSparseCluster3D>("ubooneseg");

  auto & event_cluster2d = io.get_data<larcv3::EventSparseCluster2D>("ubooneseg");
  ...
  for (auto& ch : *simch) {
    ...
    for (auto& TDCIDE : ch.TDCIDEMap()) {
      auto& tdc_pre = TDCIDE.first ;
      auto& ides = TDCIDE.second;
      //std::cout << "TDC: " << tdc <<  std::endl;
      //std::cout << "IDES: " << ides <<  std::endl;

      // I'm pretty sure that tdc is geant 4 tick so it should be shifted by some amount (how much) to match 
      // the electronics clock. 
      ...
      for (auto& ide : ides) {
        ...
        if (tdc < tick_offset || tdc > tick_offset + n_ticks_per_chamber){
          //std::cout << "Out of the time window " << std::endl;
          continue;
        }
        if (ide.trackID == -1){
          continue;
